...
fn uri_query(_input: &crate::operation::list_parts::ListPartsInput, mut output: &mut ::std::string::String) -> ::std::result::Result<(), ::aws_smithy_types::error::operation::BuildError> {
    ...
    if let ::std::option::Option::Some(inner_4) = &_input.max_parts {
         {
            query.push_kv("max-parts", &::aws_smithy_http::query::fmt_string(&inner_4));
        }
    }
    if let ::std::option::Option::Some(inner_5) = &_input.part_number_marker {
         {
            query.push_kv("part-number-marker", &::aws_smithy_http::query::fmt_string(&inner_5));
        }
    }
    let inner_6 = &_input.upload_id;
    let inner_6 = inner_6.as_ref().ok_or_else(|| ::aws_smithy_types::error::operation::BuildError::missing_field("upload_id", "cannot be empty or unset"))?;
    if inner_6.is_empty() {
